...
bool remCompare(Date *a, Date *b) {
    ...
    if (*a < *b) {
        ...
        PriorityReminder * tmp1; // tmp pointer for a
        PriorityReminder * tmp2; // tmp pointer for b
        tmp1  = dynamic_cast<PriorityReminder *>(a);
        tmp2  = dynamic_cast<PriorityReminder *>(b);
        if (tmp1 != 0 && tmp2 == 0){
            return true;
        }
